...
sorttable = {
	...
	guessType : function(table, column) {
		...
		for ( var i = 0; !typeFound && i < table.tBodies[0].rows.length; i++) {
			var text = sorttable
					.getInnerText(table.tBodies[0].rows[i].cells[column]);
			if (text != '') {
				if (text.match(/^-?[£$¤]?[\d,.]+%?$/)) {
					type = sorttable.sort_numeric;
				}
				else {
					...
					if (possdate) {
						...
						if (first > 12) {
							// definitely dd/mm
							type = sorttable.sort_ddmm;
						} else if (second > 12) {
							type = sorttable.sort_mmdd;
						} else {
